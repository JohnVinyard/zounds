...
class TemplateMatch(SingleInput):
    ...
    def __init__(self,needs = None, key = None, nframes = None, step = None,
        ...
        self.activation = \
            self._soft_activation if soft_activation else self._hard_activation
    ...
    def toeplitz2d(self,patch,size,silence_thresh):
        '''
    ...
    def feature(self,patch,ci):
        size = self._sizes[ci]
        thresh = self._silence_thresh[ci]
        mat,nz = self.toeplitz2d(patch, size, thresh)
        print '%i of %i' % (len(nz),len(mat))
        if not len(nz):
            return np.zeros(self._dim)
        dist = cdist(mat[nz],self._codebooks[ci])
        return self.activation(dist)
    
    def _soft_activation(self,dist):
        '''
    ...
    def _hard_activation(self,dist):
        '''
        Pass the soft activation through a hard thresholding function
        '''
        act = self._soft_activation(dist)
        return act <= self._hard_activation_thresh
    
    
    def _process(self):
        data = np.array(self.in_data[:self.nframes]).reshape(self._inshape)
        return np.concatenate([self.feature(data,i) for i in xrange(self._nbooks)])
