...
class Scatter(SingleInput):
    ...
    def setup_filterbank(self):
        ...
        for i in xrange(nresolutions):
            ...
            if 0 == i:
                n = np.zeros(size)
                for j in range(offset,self.log_spaced_filters):
                    n += np.abs(bank[i][j]**2)
                normal = np.sqrt(2/np.max(n))
                
                # renormalize the previously computed features
                for q in range(offset,self.total_filters):
                    bank[i][q] *= normal
    ...
    def decompose(self,sig):
        ...
        for j in range(max(0,nb),self.total_filters):
    ...
    def _process(self):
        ...
        out = safe_log(np.abs(np.array(out[2])))
        return out
