...
class Bucket(object):
    ...
    def abbreviated(self):
        return self.recarray[:-self.largest_offset]
    
    # TODO: Once I've filled up self so that it's non-jagged, append as much
    # of other as possible, so that self stays non-jagged.
    def combine(self,other):
        '''
        Modifies self and other so that self is non-jagged, and other is jagged.
        '''
        if self.clean:
            return self,other
