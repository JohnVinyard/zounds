...
from flow import \
    Node,BaseModel,ByteStream,ByteStreamFeature,Feature,Registry,\
    IdProvider,UuidProvider,Database,InMemoryDatabase,KeyBuilder,\
    StringDelimitedKeyBuilder,DataWriter
...
class FuzzTests(unittest2.TestCase):
    ...
    def __repr__(self):
        return 'FuzzTests(sr = {_samplerate}, fmt = {_fmt}, st = {_subtype}, secs = {_seconds}, ch = {_channels}, cs = {_chunksize_bytes})'.format(**self.__dict__)
...
def suite():
    ...
    for _ in xrange(10):
        ...
        suite.addTest(\
              FuzzTests(chunksize,samplerate,fmt[0],fmt[1],channels,seconds))    
    return suite
